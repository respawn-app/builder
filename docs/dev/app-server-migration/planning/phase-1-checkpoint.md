...
- `server/bootstrap`, `server/runprompt`, `server/launch`, and `server/runtimewire` now own the first real server-side launch/runtime path, but `cli/app` still owns auth/onboarding interaction flow, interactive lifecycle mutations, and runtime-native UI adapters that need further extraction in later Phase 1 slices.
