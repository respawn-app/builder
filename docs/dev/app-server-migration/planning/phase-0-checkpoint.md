...
| Ask/approval lifecycle | `internal/app/ask_bridge_test.go` for the current synchronous bridge; `internal/app/run_prompt_test.go`: headless ask prohibition; `internal/tools/askquestion/tool_test.go`: queued ask blocking, pending visibility, single completion, and duplicate-resolution rejection; `internal/app/patch_outside_workspace_approver_test.go`: queued approval blocking, deny path, and `allow_session` caching without the TUI | Current Phase 0 proof now covers non-UI queued ask/approval lifecycle in broker memory. It does not prove restart-durable pending ask/approval state, and current evidence points to broker-memory-only behavior today. Treat restart durability as a target-architecture decision, not as an already-preserved invariant. |
...
- none in the current monolith characterization backlog; remaining open items are target-architecture decisions and later client-boundary proof work

Ask/approval durability clarification after the non-UI lifecycle batch:

- current proof covers broker-memory pending asks/approvals and deterministic single-completion behavior outside the TUI
- there is still no evidence that pending ask/approval state survives process restart or reopen in today's monolith
- if restart durability is required for the app-server target, lock that as a target behavior explicitly rather than treating it as current behavior that must already be preserved
- that target-architecture decision remains explicitly open in `../spec/open-questions.md` and should stay visible during planning even though the monolith characterization backlog is now materially closed
