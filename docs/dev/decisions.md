...
- Detail scrolling is line-oriented. `Up`/`Down` move by one rendered line when the viewport can scroll and preserve the selected item while it remains visible; at transcript boundaries they move selection within the visible entries. `PgUp`/`PgDn` and wheel/alternate-scroll events keep first-visible selection so `Enter` toggles the item at the top of the viewport after coarse scrolls.
