...
- The default WebSocket transport remains `golang.org/x/net/websocket` after the 2026-04-19 `gws` spike. Builder-path benchmarks showed `gws` helps steady-state persistent unary calls, especially over the same-machine Unix socket path, but it does not clearly beat the current adapter on dial/handshake and full attach cost. Keep `gws` additive behind `shared/rpcwire` until a broader parity matrix and Builder-specific measurements justify a switch.
