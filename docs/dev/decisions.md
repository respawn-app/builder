...
- `cache_warning_mode` is a three-state config: `off` disables cache warnings, `default` shows exact same-key non-postfix warnings only, and `verbose` additionally shows provider-reported cache reuse drops for postfix-compatible requests when the provider does not expose the cause.
