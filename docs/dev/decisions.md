...
- Detail scrolling is line-oriented: `Up`/`Down` move by one rendered line, `PgUp`/`PgDn` move by page, and wheel/alternate-scroll events follow the same line-scroll path. After scrolling, compact detail selects the first visible selectable item so `Enter` toggles the item at the top of the viewport without jumping over large expanded outputs.
...
- Detail items are separated by one blank line, not a divider rule. Divider rules stay reserved for ongoing/native transcript role-group boundaries.
