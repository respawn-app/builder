...
- Goal mode requires `ask_question` for active model loops. Validate active-goal/question parity at the boundary that starts model work and surface a normal runtime error if violated; do not add queue-specific or config-specific special cases.
...
- While a model turn is currently running, TUI goal lifecycle only accepts pause and clear. Set/replacement, resume, and complete are rejected while a model turn is currently running.
