...
cache_warning_mode = "default" # or "verbose" / "off"
...
| `cache_warning_mode` | string | `default` | `BUILDER_CACHE_WARNING_MODE` |  | Prompt-cache warning policy. Allowed: `off`, `default`, `verbose`. `default` only warns when Builder proves that a same-key request was not a postfix extension of the previous request. `verbose` also warns when provider-reported cache reuse disappears for a postfix-compatible request and the provider does not expose the cause. |
