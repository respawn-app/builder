...
cache_warning_mode = "default" # conservative; or "verbose" / "off"
...
| `cache_warning_mode` | string | `default` | `BUILDER_CACHE_WARNING_MODE` |  | Prompt-cache warning policy. Allowed: `off`, `default`, `verbose`. `default` is the conservative policy and only shows provider- or runtime-authoritative warnings. `verbose` also surfaces exact same-key non-postfix rewrites, compaction-caused invalidations, and provider-reported cache reuse drops for postfix-compatible requests when the provider does not expose the cause. |
