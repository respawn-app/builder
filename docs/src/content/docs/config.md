...
| `context_compaction_threshold_tokens` | int | `258400` | `BUILDER_CONTEXT_COMPACTION_THRESHOLD_TOKENS` |  | Auto-compaction threshold. Must be `> 0` and `< model_context_window`. The default is derived from the default context window. When auto-compaction is enabled, Builder also emits a non-configurable reminder at 85% of this threshold. |
