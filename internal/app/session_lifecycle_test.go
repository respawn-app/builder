...
func TestBackTeleportLifecycleSeedsParentDraftWithoutAutoSubmit(t *testing.T) {
	tests := []struct {
		...
		{name: "copy latest child final assistant reply past typed developer metadata", childMessages: []llm.Message{{Role: llm.RoleAssistant, Content: "test", Phase: llm.MessagePhaseFinal}, {Role: llm.RoleDeveloper, MessageType: llm.MessageTypeErrorFeedback, Content: "phase mismatch"}}, childActivity: uiActivityIdle, want: "test"},
