...
func isDetailOnlyRole(role string) bool {
	switch strings.ToLower(strings.TrimSpace(role)) {
	case "thinking", "thinking_trace", "reasoning", "compaction_summary", roleManualCompactionCarryover, "error":
		return true
	default:
		return false
	}
...
func isStyledMetaRole(role string) bool {
	...
	return isCompactionRole(trimmed) || trimmed == "reviewer_status" || trimmed == "reviewer_suggestions" || trimmed == "error"
}
