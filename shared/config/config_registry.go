...
func newSettingsRegistry() settingsRegistry {
	settings := []registrySetting{
		...
		newBoolSetting("tool_preambles", true,
			...
			settingDocOptions{}),
		newBoolSetting("cache_invalidation_warning", defaultCacheInvalidationWarning,
			func(state *settingsState, value bool) { state.Settings.CacheInvalidationWarning = value },
			func(state settingsState) bool { return state.Settings.CacheInvalidationWarning },
			"BUILDER_CACHE_INVALIDATION_WARNING",
			settingDocOptions{}),
