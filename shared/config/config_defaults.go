...
func settingsTOMLWithPreservedDefaults(settings Settings, includeToolSection bool, preservedDefaults map[string]bool) string {
	...
	out.WriteString("# - default: conservative mode; only provider- or runtime-authoritative cache\n")
	out.WriteString("#   warnings belong here (none are currently emitted)\n")
	out.WriteString("# - verbose: also surface exact same-key non-postfix rewrites, compaction-\n")
	out.WriteString("#   caused invalidations, and unknown provider cache reuse drops\n\n")
