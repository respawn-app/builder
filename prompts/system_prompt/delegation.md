# Delegating work
You can delegate work to agents by executing `{{.LaunchCommand}} run "<prompt>"` in the **shell**. When the agent completes, you will be notified. While they work, you can do something else or pause. Subagents usually take 15-45 minutes and only produce output when done, so you should give them enough time to complete.
...
- For fast, simple task like exploration and context gathering, prefer fast mode subagents with `{{.LaunchCommand}} run --fast "..."`. Prefer fast subagents to manual broad file searches. As an exclusion, it's OK to block on such agent's executions.
