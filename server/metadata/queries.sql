...
-- name: GetMutationDedupRecord :one
SELECT
    method,
    resource_id,
    client_request_id,
...
-- name: DeleteExpiredMutationDedupRecords :execrows
DELETE FROM mutation_dedupe
WHERE expires_at_unix_ms <= sqlc.arg(expires_at_unix_ms);
